...
export function PersonalPlace({ profile }: { profile: PublicProfile }) {
  const { brandName } = useBranding();
  const name = profile.name?.trim() || "Digital card";
  const title = profile.tagline?.trim() || "";
  const company = brandName && brandName.toLowerCase() !== name.toLowerCase() ? brandName : "";
  ...
  const contacts = [
    ...
    profile.contact.whatsapp && wa
      ? { href: wa, label: profile.contact.whatsapp, caption: "main", icon: Phone, external: true }
      : null,
  ...
  return (
    <div className="min-h-[100svh] bg-[#F4F5F8] text-[#111827]">
      ...
      <div className="mx-auto w-full max-w-[430px] px-6 pb-16">
        ...
        {about && <p className="mt-6 text-[15px] leading-7 text-[#374151] whitespace-pre-line">{about}</p>}

        <button
          type="button"
          onClick={() => downloadTextFile(`${name}.vcf`, buildVCard(profile), "text/vcard")}
          className="mt-7 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[#111827] text-[13px] font-semibold uppercase tracking-[0.14em] text-white"
        >
        ...
        {gallery.length > 0 && (
          <section className="mt-10">
            ...
            <h2 className="mt-1 text-xl font-semibold tracking-tight">Selected work</h2>
